    ...
    def __init__(
        ...
        if self.cls in [SCGM]:
            ...
            self.datasets = get_split_dataset(
                ...
                mode,
                1.0,
                #get_mnms_domains if self.cls is MNMS else get_scgm_domains,
                self.cls,  # type: ignore
            )
        else:
            if mode == "val":
                assert len(target_domain) == 1
                self.datasets = [self.cls(root, target_domain[0], "val")]
            else:
                self.datasets = [
