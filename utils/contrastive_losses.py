...
def contrastive_class_to_class_learned_memory(model, features, class_labels,
                                              num_classes, memory):
    """
    ...
    for c in range(num_classes):
        ...
        selector_memory = model.__getattr__(
            'contrastive_class_selector_memory' + str(c))

        if memory_c is not None and features_c.shape[0] > 1 and memory_c.shape[
                0] > 1:
            ...
            similarities = torch.mm(features_c_norm,
                                    memory_c.transpose(1, 0))  # MxN
            ...
            learned_weights_features = selector(
                features_c.detach())  # detach for trainability
            ...
            learned_weights_features_memory = learned_weights_features_memory.permute(
                1, 0)
