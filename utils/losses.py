...
class mask_DiceLoss(nn.Module):
    ...
    def __init__(self, nclass, class_weights=None, smooth=1e-5):
        ...
        if class_weights is None:
            ...
            self.class_weights = nn.Parameter(torch.ones(
                (1, nclass)).type(torch.float32),
                                              requires_grad=False)
        else:
            class_weights = np.array(class_weights)
            assert nclass == class_weights.shape[0]
            self.class_weights = nn.Parameter(torch.tensor(
    ...
    def prob_forward(self, pred, target, mask=None):
        ...
        target_one_hot = to_one_hot(target.type(torch.long),
                                    nclass).type(torch.float32)
    ...
    def forward(self, inputs, target, mask=None, weight=None, softmax=False):
        ...
        if mask is not None:
            ...
            for i in range(0, self.n_classes):
                dice = self._dice_mask_loss(inputs[:, i], target[:, i],
                                            mask[:, i])
...
def to_one_hot(tensor, nClasses):
    ...
    ) < nClasses, 'one hot tensor.max() = {} < {}'.format(
...
def _l2_normalize(d):
    ...
    d /= torch.norm(d_reshaped, dim=1,
                    keepdim=True) + 1e-8  # 2-p length of vector
    return d


class VAT2d(nn.Module):

    def __init__(self, n_classes: int, xi=10.0, epi=6.0, ip=1):
...
class VAT3d(nn.Module):
    ...
    def forward(self, model, x):
        ...
        # initialize a random tensor between [-0.5, 0.5]
        d = torch.rand(x.shape).sub(0.5).to(x.device)
...
def l_correlation_cos_mean(model1, model2,
                           linear_params: nn.ModuleList) -> torch.Tensor:
    total_loss = 0.0
    count = 0
    for name, parameters in model1.named_parameters():
        if 'conv' in name and 'weight' in name:
            # if len(parameters.shape) == 5:
            if parameters.ndim == 4:  # conv2d
                w1 = parameters
